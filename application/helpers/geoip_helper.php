...
function getUserCountry($ip = false)
{
    ...
    $geo = getDataByIp($ip);
    if (isset($geo->country->name_ru)) return $geo->country->name_ru;

    return false;
}

function getUserCity($ip = false)
{
    getDataByIp($ip);
    if (isset($geo->city->name_ru)) return $geo->city->name_ru;

    return false;
}

function getDataByIp($ip = false)
{
    if (!$ip)
        $ip = GetRealIp();
...
function getTabGeo($ip = false)
{
    if (!$ip)
        $ip = GetRealIp();
    $fh = fopen($_SERVER['DOCUMENT_ROOT'] . '/upload/geo/tabgeo_country_v4.dat', 'rb');
